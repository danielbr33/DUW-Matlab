...
for i=1:n
    ...
    Polozenia(1:2,i)=q(3*n-2:3*n-1)+Rot(q((3*n)))*sA;
    Polozenia(3,i)=q((3*n));
    Predkosci(1:2,i)=dq(3*n-2:3*n-1)+Om*Rot(dq(3*n))*sA*dq(3*n);
    Predkosci(3,i)=dq(3*n);
    Przyspieszenia(1:2,i)=d2q(3*n-2:3*n-1)+Om*Rot(q(3*n))*sA*d2q(3*n)-Rot(q(3*n))*sA*dq(3*n)*dq(3*n);
    Przyspieszenia(3,i)=d2q(3*n);
